...
- Emits canonical URL, full Open Graph + Twitter card tags, robots directives, and JSON-LD. It always
  injects `Organization` + `WebSite` schema, merges any page `jsonLd`, and auto-builds a `FAQPage`
  schema from `faqSchema`.
- `SITE_URL` is `https://emlinter.app`. Update there if the domain changes.
...
Every React mount uses `client:only="react"` — **not** `client:load` or `client:visible`. The components
rely on `window`, `localStorage`, the Firebase client SDK, and client-only React state that does not
survive SSR. Do not change a mount directive without first confirming the component is SSR-safe.
...
- `VisualEditorPage` backs both `/visual-editor` and `/visual-editor/<slug>`.
...
- `firebase.ts` — Firestore reads/writes. Exports: `getTemplates`, `getTemplateBySlug`,
  ...
  Firebase web config is inline and public — safe to commit.
...
- **New route** → add a `.astro` shell under `src/pages/` that imports the React page from
  ...
  `jsonLd`), and mounts it `client:only="react"`. Add a row to the route map above.
...
1. `npm run build` — must exit 0 and print `Complete!`. This is the fastest way to catch a broken
   import, bad `.astro` frontmatter, or a type error.
...
3. Firebase-backed pages need network + a valid project; Gemini features need `PUBLIC_GEMINI_API_KEY`.
