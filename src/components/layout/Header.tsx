...
const Header: React.FC<HeaderProps> = ({ activePage, onNavigate }) => {
    ...
    const handleLinkClick = (pageKey: string) => {
                ...
                <nav className="container mx-auto px-4 flex items-center justify-between h-20">
                    ...
                    <ul className="hidden md:flex items-center space-x-6 md:space-x-8">
                        {navLinks.map(({ page, label, children }) => {
                                        ...
                                        {openDropdown === page && (
                                            <div onMouseEnter={() => handleDropdownEnter(page)} onMouseLeave={handleDropdownLeave} className="absolute top-full left-1/2 -translate-x-1/2 mt-3 bg-gray-800/80 backdrop-blur-md border border-gray-700 rounded-xl shadow-2xl p-4">
                                                <div className={`grid ${children.length > 4 ? 'grid-cols-2 w-[40rem]' : 'grid-cols-1 w-[20rem]'} gap-x-4 gap-y-2`}>
                                                    {children.map(child => (
                                                        <a key={child.page} href={`/${child.page}`} onClick={(e) => { e.preventDefault(); handleLinkClick(child.page); }} className="group flex items-start gap-4 p-3 rounded-lg hover:bg-gray-700/50 transition-colors">
                            ...
                            return (
                                <li key={page}>
                                    <a href={path} onClick={(e) => { e.preventDefault(); handleLinkClick(page); }} className={getDesktopLinkClass(page)}>
                                        {label}
                                    </a>
                                </li>
                            );
            ...
            >
                <ul className="flex flex-col items-center space-y-10 text-center">
                    {navLinks.map(({ page, label, children }) => {
                                ...
                                <li key={page}>
                                    ...
                                    {mobileDropdownsOpen[page] && (
                                        <div className="mt-4 space-y-4">
                                            {children.map(child => (
                                                <a key={child.page} href={`/${child.page}`} onClick={(e) => { e.preventDefault(); handleLinkClick(child.page); }} className={`block text-xl font-medium ${activePage === child.page ? 'text-pink-400' : 'text-gray-400'} hover:text-white`}>
                                                    {child.label}
                                                </a>
                                            ))}
                                        </div>
                        ...
                        return (
                            <li key={page}>
                                <a href={path} onClick={(e) => { e.preventDefault(); handleLinkClick(page); }} className={getMobileLinkClass(page)}>
                                    {label}
                                </a>
                            </li>
                        );
