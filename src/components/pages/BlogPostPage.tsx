...
const RecommendedPostsSidebar: React.FC<{ posts: Post[]; currentPostId: string; loading: boolean }> = ({ posts, currentPostId, loading }) => (
    <aside className="lg:col-span-1">
        <div className="bg-gray-800/50 rounded-xl p-4 border border-gray-700 sticky top-24">
            <h2 className="text-lg font-bold flex items-center gap-2 mb-4"><TrendingUpIcon className="w-6 h-6 text-pink-400" /><span>Recommended Reads</span></h2>
            {loading ? (
                <div className="space-y-4">{[...Array(5)].map((_, i) => <div key={i} className="animate-pulse flex flex-col"><div className="h-4 bg-gray-700 rounded w-5/6 mb-2"></div><div className="h-3 bg-gray-700 rounded w-1/2"></div></div>)}</div>
            ) : (
                <ul className="space-y-3">
                    {posts.filter(p => p.id !== currentPostId).map(post => (
                        <li key={post.id}>
                            <a href={`/resources/blog/${post.slug}`} className="text-left w-full group block">
...
const BlogPostPage: React.FC<{ slug: string }> = ({ slug }) => {
    ...
    useEffect(() => {
        const fetchPostData = async () => {
            try {
                setLoading(true); setLoadingRecs(true); setError(null); setVotedStatus(null);
                const [fetchedPost, recPosts] = await Promise.all([getPostBySlug(slug), getRecommendedPosts()]);
                if (fetchedPost) {
                    setPost(fetchedPost);
                    ...
                    if (storedVote === 'helpful' || storedVote === 'not-helpful') setVotedStatus(storedVote as 'helpful' | 'not-helpful');
                    document.title = `${fetchedPost.title} | EMLinter`;
                    const metaDesc = document.querySelector('meta[name="description"]');
                    if (metaDesc && fetchedPost.seoMetaDescription) metaDesc.setAttribute('content', fetchedPost.seoMetaDescription);
                } else { setError("Blog post not found."); }
                setRecommendedPosts(recPosts);
            } catch (err) { console.error(err); setError("Failed to load the blog post."); }
            finally { setLoading(false); setLoadingRecs(false); }
        };
        fetchPostData();
    }, [slug]);
    ...
    return (
        <div className="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-3 gap-8 lg:gap-12">
            <article className="lg:col-span-2">
                <style>{`
                    .prose{color:#D1D5DB}.prose h1,.prose h2,.prose h3,.prose h4,.prose h5,.prose h6{color:#F9FAFB;margin-top:1.5em;margin-bottom:0.8em}
                    .prose h1{font-size:2.25rem}.prose h2{font-size:1.875rem}.prose h3{font-size:1.5rem}
                    .prose a{color:#EC4899;text-decoration:underline;transition:color 0.2s}.prose a:hover{color:#F472B6}
                    ...
                    .prose li p{margin:0}.prose ul li{position:relative;padding-left:1.75rem;margin-bottom:0.75rem}
                    ...
                    .prose ol{counter-reset:ordered-list-counter}.prose ol li{position:relative;padding-left:2.5rem;margin-bottom:0.75rem;counter-increment:ordered-list-counter}
                    ...
                    .prose img{width:100%;border-radius:0.75rem;box-shadow:0 10px 15px -3px rgba(0,0,0,0.1);margin-top:2em;margin-bottom:2em}
                `}</style>
                <header className="mb-8">
                    <a href="/resources/blog" className="text-sm font-semibold text-pink-400 hover:text-pink-300 transition-colors mb-4 inline-block">&larr; Back to Blog</a>
                    <div className="flex flex-wrap gap-2 mb-4">{post.categories?.map(cat => <span key={cat} className="text-xs bg-violet-800/50 text-violet-300 font-semibold px-3 py-1 rounded-full">{cat}</span>)}</div>
                    <h1 className="text-4xl md:text-5xl font-extrabold text-gray-50 leading-tight">{post.title}</h1>
                ...
                <div className="mt-12 pt-8 border-t border-gray-700/50 text-center">
                    ...
                    <div className="flex justify-center items-center gap-4">
                        <button onClick={() => handleVote('helpful')} disabled={!!votedStatus || isVoting}
                            className={`flex items-center gap-2 px-6 py-2 font-semibold rounded-lg transition-all duration-200 disabled:cursor-not-allowed group ${votedStatus === 'helpful' ? 'bg-green-600 text-white ring-2 ring-green-400' : 'bg-gray-700 text-gray-300 hover:bg-green-800/50 disabled:bg-gray-700/50'}`}>
                            ...
                            <span>Helpful</span>
                            <span className={`ml-1 px-2 py-0.5 text-xs rounded-full ${votedStatus === 'helpful' ? 'bg-green-700' : 'bg-gray-600'}`}>{votes.helpful}</span>
                        </button>
                        <button onClick={() => handleVote('not-helpful')} disabled={!!votedStatus || isVoting}
                            className={`flex items-center gap-2 px-6 py-2 font-semibold rounded-lg transition-all duration-200 disabled:cursor-not-allowed group ${votedStatus === 'not-helpful' ? 'bg-red-600 text-white ring-2 ring-red-400' : 'bg-gray-700 text-gray-300 hover:bg-red-800/50 disabled:bg-gray-700/50'}`}>
                            ...
                            <span className={`ml-1 px-2 py-0.5 text-xs rounded-full ${votedStatus === 'not-helpful' ? 'bg-red-700' : 'bg-gray-600'}`}>{votes.notHelpful}</span>
                        </button>
                    </div>
