...
import { SpinnerIcon, NewspaperIcon, TrendingUpIcon } from '../Icons';
...
const RecommendedSidebar: React.FC<{ posts: Post[]; loading: boolean }> = ({ posts, loading }) => (
...
const BlogPage = () => {
    ...
    useEffect(() => {
        Promise.all([getPosts(), getRecommendedPosts()])
            .then(([posts, recPosts]) => { setAllPosts(posts); setRecommendedPosts(recPosts); })
            .catch(err => { console.error(err); setError("Failed to load blog posts."); })
            .finally(() => { setLoading(false); setLoadingRecs(false); });
    }, []);

    const totalPages = Math.ceil(allPosts.length / ITEMS_PER_PAGE);
    const paginatedPosts = useMemo(() => allPosts.slice((currentPage - 1) * ITEMS_PER_PAGE, currentPage * ITEMS_PER_PAGE), [allPosts, currentPage]);
    ...
    return (
        <div>
            <header className="text-center mb-10">
                <h1 className="text-4xl md:text-5xl font-bold bg-clip-text text-transparent bg-gradient-to-r from-pink-500 to-violet-500 pb-2">EMLinter Blog</h1>
                <p className="text-gray-400 mt-2 max-w-2xl mx-auto">Insights, tutorials, and updates on HTML email development from the EMLinter team.</p>
            </header>
            <div className="grid grid-cols-1 lg:grid-cols-4 gap-8">
                            ...
                            {totalPages > 1 && (
                                <nav className="mt-12 flex justify-center items-center gap-2">
                                    {Array.from({ length: totalPages }, (_, i) => i + 1).map(page => (
                                        <button key={page} onClick={() => setCurrentPage(page)} className={`w-10 h-10 rounded-md text-sm font-semibold transition-colors ${currentPage === page ? 'bg-pink-600 text-white' : 'bg-gray-800 text-gray-300 hover:bg-gray-700'}`}>{page}</button>
                                    ))}
                                </nav>
                            )}
                        </>
                ...
                <RecommendedSidebar posts={recommendedPosts} loading={loadingRecs} />
            </div>
        </div>
    );
};
