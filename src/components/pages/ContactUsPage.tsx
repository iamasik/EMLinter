...
import { CopyIcon, FacebookIcon, TwitterIcon, InstagramIcon, YoutubeIcon } from '../Icons';
...
const ContactUsPage = () => {
    ...
    useEffect(() => {
        setLoading(true);
        getAppSettings().then(setSettings).catch(console.error).finally(() => setLoading(false));
    }, []);
        ...
        <>
            ...
            <div className="relative overflow-hidden py-12">
                ...
                <header className="text-center mb-16 opacity-0 animate-enter" style={{ animationDelay: '0s' }}>
                    ...
                    <p className="text-gray-400 mt-4 max-w-2xl mx-auto">Whether you have a question, a project idea, or just want to say hi, I'd love to hear from you.</p>
                </header>
                                ...
                                {loading ? (
                                    ...
                                    <>
                                        <div className="flex-grow mb-6 space-y-2">
                                            {settings.contact.email && <p className="text-gray-400 text-sm break-all"><span className="font-semibold text-gray-300">Email:</span> {settings.contact.email}</p>}
                                            {settings.contact.phone && <p className="text-gray-400 text-sm"><span className="font-semibold text-gray-300">Call:</span> {settings.contact.phone}</p>}
                                        </div>
                                    ...
                                    <div className="flex-grow mb-6"><p className="text-gray-500 text-sm">Contact information is currently unavailable. Please check back later or use our social media channels.</p></div>
                                )}
                            </div>
                        </div>
                    </div>
                        ...
                        <div className="relative p-8 h-full rounded-2xl bg-gray-800/50 border border-gray-700/50 overflow-hidden transform transition-transform duration-300 hover:-translate-y-2">
                            ...
                            <div className="relative z-10 flex flex-col h-full">
                                ...
                                <div className="mt-auto flex justify-center items-center space-x-6">
                                    ...
                                    <a href="https://twitter.com" target="_blank" rel="noopener noreferrer" className="text-gray-400 hover:text-white transition-colors" aria-label="Twitter"><TwitterIcon className="w-6 h-6" /></a>
                                    <a href="https://instagram.com" target="_blank" rel="noopener noreferrer" className="text-gray-400 hover:text-white transition-colors" aria-label="Instagram"><InstagramIcon className="w-6 h-6" /></a>
                                    <a href="https://youtube.com" target="_blank" rel="noopener noreferrer" className="text-gray-400 hover:text-white transition-colors" aria-label="YouTube"><YoutubeIcon className="w-6 h-6" /></a>
                                </div>
                            </div>
                        </div>
                    </div>
