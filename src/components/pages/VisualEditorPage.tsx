...
const VisualEditorPage: React.FC<VisualEditorPageProps> = ({ slug }) => {
    ...
    const previewWidth = useMemo(() => viewMode === 'mobile' ? 375 : 800, [viewMode]);
     ...
     // This useEffect will manage the history stack
    useEffect(() => {
        // If the change came from an undo/redo action, don't create a new history state.
        ...
        setHistory(prevHistory => {
            const currentContent = prevHistory.stack[prevHistory.index];
            if (templateHtml === currentContent) {
                return prevHistory; // No change, don't add to history
            }

            // If we've undone actions, we need to clear the "future" history.
            const newStack = prevHistory.stack.slice(0, prevHistory.index + 1);
            newStack.push(templateHtml || ''); // Add the new state.
            
            return {
                stack: newStack,
                index: newStack.length - 1 // Move the pointer to the new latest state.
            };
        });
    }, [templateHtml]);
        ...
        const loadTemplate = async () => {
            if (slug) {
                ...
                try {
                    ...
                    if (savedHtml) {
                        setHistory({ stack: [savedHtml], index: 0 });
    ...
    const handleUndo = () => {
        if (history.index > 0) {
            ...
            setTemplateHtml(history.stack[newIndex]);
        }
    };
    ...
    useEffect(() => {
        const handleKeyDown = (event: KeyboardEvent) => {
            if (event.ctrlKey || event.metaKey) { // Ctrl on Windows, Cmd on Mac
                if (event.key === 'z') {
    ...
    }, [history]); // Dependency on history ensures the handler has the latest state
                                ...
                                <>
                                    <div className="flex items-center gap-1 bg-gray-700 p-1 rounded-lg">
                                        <button
                                            ...
                                            className="p-2 rounded-md text-sm font-semibold transition-colors text-gray-300 hover:bg-gray-600 disabled:text-gray-500 disabled:cursor-not-allowed"
                                            title="Undo (Ctrl+Z)"
                                        >
