...
import { InfoIcon, CodeIcon, DownloadIcon, AlertTriangleIcon, SaveIcon, SpinnerIcon, CheckCircleIcon, TrashIcon, DesktopIcon, MobileIcon, BriefcaseIcon, CloseIcon, UploadIcon } from '../components/Icons';
...
const VisualEditorPage: React.FC<VisualEditorPageProps> = ({ slug }) => {
    ...
    useEffect(() => {
        const handleMessage = (event: MessageEvent) => {
            if (event.data.type === 'vibe-html-update' && event.data.html) {
                ...
                finalHtml = finalHtml.replace(/rgb\(\s*(\d+)\s*,\s*(\d+)\s*,\s*(\d+)\s*\)/g, (match: string) => {
    ...
    useEffect(() => {
        ...
        const handleLoad = () => {
            ...
            style.textContent = `
                ...
                .vibe-controls { position: absolute; top: -14px; left: 0px; z-index: 9999; background-color: #4C1D95; border-radius: 6px; display: none; gap: 4px; padding: 4px; box-shadow: 0 4px 6px rgba(0,0,0,0.1); }
                .vibe-section-hover .vibe-controls { display: flex; }
            ...
            const duplicateIconSvg = `<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M12 4v16m8-8H4" /></svg>`;
            ...
            const controlsHtml = 
                `<button class="vibe-control-btn" data-action="move-up" title="Move Section Up">↑</button>` +
                `<button class="vibe-control-btn" data-action="move-down" title="Move Section Down">↓</button>` +
                `<button class="vibe-control-btn" data-action="duplicate" title="Duplicate Section">${duplicateIconSvg}</button>` +
                `<button class="vibe-control-btn" data-action="delete" title="Delete Section">${deleteIconSvg}</button>`;
            ...
            script.textContent = `
                function syncHtml() {
                    ...
                    cleanDoc.querySelectorAll('[data-vibe-initialized], [data-vibe-img-id], [data-vibe-link-id], [data-vibe-text-id], [data-vibe-bg-id]').forEach(el => {
                        el.classList.remove('vibe-section-hover', 'vibe-bg-hover');
                ...
                function initSection(section) {
                    const firstCell = section.querySelector('td, th');
                    if (!firstCell || section.dataset.vibeInitialized) return;
                    section.dataset.vibeInitialized = 'true';
                    ...
                    section.addEventListener('mouseenter', () => {
                        section.classList.add('vibe-section-hover');
                        ...
                        if (moveUpBtn) {
                            ...
                            moveUpBtn.disabled = !prevSibling || !prevSibling.classList.contains('vibe-section');
                        }
                        if (moveDownBtn) {
                            const nextSibling = section.nextElementSibling;
                            moveDownBtn.disabled = !nextSibling || !nextSibling.classList.contains('vibe-section');
                        }
                    });
                    section.addEventListener('mouseleave', () => section.classList.remove('vibe-section-hover'));
                    controls.addEventListener('click', (e) => {
                        ...
                        if (action === 'duplicate') {
                            ...
                            const prevSibling = section.previousElementSibling;
                            if (prevSibling && prevSibling.classList.contains('vibe-section')) {
                                section.parentNode.insertBefore(section, prevSibling);
                            }
                        } else if (action === 'move-down') {
                            const nextSibling = section.nextElementSibling;
                            if (nextSibling && nextSibling.classList.contains('vibe-section')) {
                ...
                function initVibeElements() {
                    document.querySelectorAll('.vibe-section:not([data-vibe-initialized])').forEach(initSection);
                    let idCounter = 0;
        ...
        <>
            ...
            <div className="flex flex-col gap-8">
                ...
                <div className="bg-gray-800/50 rounded-xl shadow-2xl border border-gray-700 overflow-hidden flex flex-col">
                    ...
                    {/* Preview Area */}
                    <div className="min-h-[70vh] h-[70vh] relative bg-gray-900/50 flex justify-center items-start p-4 overflow-auto">
                         {isLoadingTemplate ? (
                            ...
                            <div 
                                className="shadow-lg transition-all duration-300 ease-in-out bg-white flex-shrink-0"
                                style={{ 
