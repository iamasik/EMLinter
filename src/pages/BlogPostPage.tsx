...
const RecommendedPostsSidebar: React.FC<{ posts: Post[], currentPostId: string, onNavigate: (path: string) => void, loading: boolean }> = ({ posts, currentPostId, onNavigate, loading }) => (
    ...
    useEffect(() => {
        const fetchPostData = async () => {
            try {
                ...
                // FIX: Replaced Promise.all with sequential awaits to avoid potential iteration-related TypeScript errors.
                ...
                if (fetchedPost) {
                    ...
                    const fullUrl = `${baseUrl}/blog/${fetchedPost.slug}`;
                    const ogImage = fetchedPost.thumbnailUrl || `${baseUrl}/og-image.png`;

                    document.title = `${fetchedPost.title} | EMLinter`;
                    setMetaTag('description', fetchedPost.seoMetaDescription, false);
                    setLinkTag('canonical', fullUrl);
                    
                    setMetaTag('og:title', fetchedPost.title);
                    setMetaTag('og:description', fetchedPost.seoMetaDescription);
                    setMetaTag('og:url', fullUrl);
                    setMetaTag('og:image', ogImage);
                    
                    setMetaTag('twitter:title', fetchedPost.title, false);
                    setMetaTag('twitter:description', fetchedPost.seoMetaDescription, false);
        ...
        <div className="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-3 gap-8 lg:gap-12">
            <article className="lg:col-span-2">
                ...
                <header className="mb-8">
                    <button onClick={() => onNavigate('/blog')} className="text-sm font-semibold text-pink-400 hover:text-pink-300 transition-colors mb-4">
                        &larr; Back to Blog
                    </button>
            ...
            <RecommendedPostsSidebar posts={recommendedPosts} currentPostId={post.id} onNavigate={onNavigate} loading={loadingRecs} />
        </div>
    );
};
