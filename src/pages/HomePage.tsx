...
const FaqItem: React.FC<{ q: string; a: React.ReactNode; onNavigate: (path: string) => void; }> = ({ q, a, onNavigate }) => {
    const [isOpen, setIsOpen] = useState(false);
    const contentRef = useRef<HTMLDivElement>(null);

    const handleA_Click = (e: React.MouseEvent<HTMLAnchorElement>) => {
    ...
    // Attach click listeners to any special links inside the answer
    useEffect(() => {
        if (contentRef.current) {
            const links = contentRef.current.querySelectorAll('a[data-page]');
            links.forEach(link => {
                link.addEventListener('click', handleA_Click as any);
            });
            return () => {
                links.forEach(link => {
                    link.removeEventListener('click', handleA_Click as any);
                });
            }
        }
    }, [a, onNavigate]);
...
const HomePage = ({ onNavigate }: { onNavigate: (path: string) => void }) => {
    ...
    const faqs = [
        ...
        },
        {
            ...
            a: <p>Yes! We have a growing <a href="/templates" data-page="/templates" className="text-pink-400 hover:underline">Template Library</a> of professionally designed, responsive HTML email templates. They are fully compatible with our tools and ready to be customized for your next campaign.</p>
        },
        {
            q: "How do I make my template editable in the Visual Editor?",
            a: <p>It's simple! You just need to add specific CSS classes like <code>vibe-section</code>, <code>vibe-text</code>, and <code>vibe-img</code> to your HTML elements. We provide a full guide on how to prepare your templates. Visit our <a href="/how-it-works" data-page="/how-it-works" className="text-pink-400 hover:underline">How It Works</a> page for a detailed walkthrough.</p>
        }
    ];
    ...
    return (
        <div className="space-y-24 md:space-y-32">
            ...
            <section className="text-center py-16 md:py-24 relative">
                ...
                <h1 className="text-5xl md:text-7xl font-extrabold animated-gradient-text mb-6 animate-on-scroll pb-4">
                ...
                <div className="flex flex-col sm:flex-row justify-center gap-4 animate-on-scroll delay-2">
                    <button onClick={() => onNavigate('/visual-editor')} className="px-8 py-3 font-semibold text-white bg-gradient-to-r from-pink-600 to-violet-600 rounded-lg hover:from-pink-700 hover:to-violet-700 transform hover:scale-105 transition-transform duration-200 shadow-lg">
                        Get Started for Free
                    </button>
                    <button onClick={() => onNavigate('/templates')} className="px-8 py-3 font-semibold text-white bg-gray-700 rounded-lg hover:bg-gray-600 transition-colors duration-200">
                        Browse Templates
                    </button>
                </div>
            </section>
            ...
            <section className="space-y-16">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                    ...
                    <div className="feature-card p-8 rounded-xl animate-on-scroll delay-1">
                        ...
                        <button onClick={() => onNavigate('/code-fix')} className="font-semibold text-pink-400 hover:text-pink-300 transition-colors">Validate Your Email &rarr;</button>
                    </div>
                    ...
                    <div className="feature-card p-8 rounded-xl animate-on-scroll delay-2">
                        ...
                        <button onClick={() => onNavigate('/visual-editor')} className="font-semibold text-violet-400 hover:text-violet-300 transition-colors">Launch the Editor &rarr;</button>
                    </div>
                    ...
                    <div className="feature-card p-8 rounded-xl animate-on-scroll delay-3">
                        ...
                        <button onClick={() => onNavigate('/templates')} className="font-semibold text-cyan-400 hover:text-cyan-300 transition-colors">Browse Templates &rarr;</button>
                    </div>
                    ...
                    <div className="feature-card p-8 rounded-xl animate-on-scroll delay-1">
                        ...
                        <button onClick={() => onNavigate('/outlook-button-generator')} className="font-semibold text-green-400 hover:text-green-300 transition-colors">Explore Outlook Tools &rarr;</button>
                    </div>
                    ...
                    <div className="feature-card p-8 rounded-xl animate-on-scroll delay-2">
                        ...
                        <button onClick={() => onNavigate('/beautify-code')} className="font-semibold text-yellow-400 hover:text-yellow-300 transition-colors">Beautify My Code &rarr;</button>
                    </div>
                    ...
                    <div className="feature-card p-8 rounded-xl animate-on-scroll delay-3">
                        ...
                        <button onClick={() => onNavigate('/how-it-works')} className="font-semibold text-gray-300 hover:text-white transition-colors">Learn More &rarr;</button>
                    </div>
                </div>
            </section>
            ...
            <section className="max-w-4xl mx-auto animate-on-scroll">
                 ...
                 <div className="mt-8">
                     {faqs.map((faq, index) => <FaqItem key={index} q={faq.q} a={faq.a} onNavigate={onNavigate} />)}
                 </div>
            </section>
            ...
            <section className="max-w-4xl mx-auto text-center animate-on-scroll">
                <div className="bg-gray-800/50 rounded-2xl border border-gray-700/50 p-10 md:p-16 relative overflow-hidden">
                    ...
                    <div className="relative z-10">
                        ...
                        <button onClick={() => onNavigate('/how-it-works')} className="px-8 py-3 font-semibold text-white bg-gray-700 rounded-lg hover:bg-gray-600 transition-colors duration-200">
                            Watch the Guides
                        </button>
                    </div>
                </div>
            ...
            <section className="py-20 text-center animate-on-scroll">
                ...
                <button onClick={() => onNavigate('/visual-editor')} className="px-10 py-4 font-semibold text-lg text-white bg-gradient-to-r from-pink-600 to-violet-600 rounded-lg hover:from-pink-700 hover:to-violet-700 transform hover:scale-105 transition-transform duration-200 shadow-lg">
                    Start for Free
                </button>
            </section>
        </div>
