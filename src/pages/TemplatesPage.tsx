...
// Fix: Changed TemplateCard to be of type React.FC to correctly handle the 'key' prop and updated onNavigate prop type.
const TemplateCard: React.FC<{ template: Template; onNavigate: (path: string, state?: any) => void; key?: any }> = ({ template, onNavigate }) => {
  return (
    <a 
      href={`/templates/${template.slug}`}
      onClick={(e) => { e.preventDefault(); onNavigate(`/templates/${template.slug}`); }}
      ...
      <div className="p-4 flex flex-col flex-grow">
        ...
        <div className="mt-3 flex flex-wrap gap-2 pt-2 border-t border-gray-700/50">
          {template.categories.slice(0, 2).map(cat => (
            <span key={cat} className="text-xs bg-gray-700 text-gray-300 px-2 py-1 rounded-full">{cat}</span>
          ))}
        </div>
      </div>
    </a>
...
const TemplatesPage = ({ onNavigate, initialFilter }: { onNavigate: (path: string, state?: any) => void, initialFilter: any | null }) => {
    ...
    const [ratingFilter, setRatingFilter] = useState<number>(0); // 0 for all ratings
    ...
    useEffect(() => {
        if (initialFilter && initialFilter.type && initialFilter.value) {
            ...
            setActiveFilters({
             ...
             setRatingFilter(0); // Also reset rating filter
        }
    }, [initialFilter]);
    ...
    }, [allTemplates, searchTerm, activeFilters, ratingFilter]);
                ...
                <main className="flex-1 min-w-0">
                    <div className="mb-6 relative">
                         <input
                            ...
                            className="w-full pl-10 pr-4 py-3 bg-gray-800/50 border border-gray-700 rounded-lg focus:ring-2 focus:ring-pink-500 focus:outline-none"
                        />
                        <svg className="w-5 h-5 text-gray-400 absolute top-1/2 left-3 transform -translate-y-1/2" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z" /></svg>
                    </div>
