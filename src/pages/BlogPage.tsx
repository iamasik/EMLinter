...
const PostCard: React.FC<{ post: Post; onNavigate: (path: string) => void; }> = ({ post, onNavigate }) => {
    return (
        <a 
            href={`/blog/${post.slug}`}
...
const RecommendedPostsSidebar: React.FC<{ posts: Post[], onNavigate: (path: string) => void, loading: boolean }> = ({ posts, onNavigate, loading }) => (
...
const BlogPage = ({ onNavigate }: { onNavigate: (path: string) => void }) => {
    ...
    return (
        <div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-4 gap-8">
                ...
                <RecommendedPostsSidebar posts={recommendedPosts} onNavigate={onNavigate} loading={loadingRecs} />
            </div>
        </div>
    );
};
